...

{
    ...
    protected function load_config()
    {
        ...
        if (!file_exists($config_file)) {
            trigger_error('[Green] Unable to locate the config file at: ' . $config_file);
        }
    ...
    {
        if (!function_exists('get_field') && !get_field('header_logo', 'option')) {
            return false;
        }
    ...
    {
        if ($this->config('favicon') && file_exists(trailingslashit(get_template_directory()) . $this->config('favicon'))) {
            printf(
                '<link rel="shortcut icon" href="%s?v=%s">',
                trailingslashit(get_template_directory_uri()) . $this->config('favicon'),
                filemtime(trailingslashit(get_template_directory()) . $this->config('favicon'))
            );
        }

        if ($this->config('phone_icon') && file_exists(trailingslashit(get_template_directory()) . $this->config('phone_icon'))) {
            printf(
                '<link rel="apple-touch-icon" href="%s?v=%s">',
                trailingslashit(get_template_directory_uri()) . $this->config('phone_icon'),
    ...
    {
        $plugins['fs_typekit'] = get_template_directory_uri() . '/js/tinymce.js';
        return $plugins;
    }
     ...
     * @return array
     */
    private function recursive_wp_parse_args(&$a, $b ): array
    {
        $a = (array) $a;
        $b = (array) $b;
        $result = $b;
    ...
    {
        return $this->recursive_wp_parse_args($props, [
            'labels' => $labels,
            'public' => true,
            'menu_position' => 46,
    ...
    protected function enqueue_scripts($javascripts, $stylessheets)
    {
        if ($javascripts) {
            foreach ($javascripts as $name => $path) {
                ...
                wp_enqueue_script(
                    $name,
                    $isExternal ? $path : trailingslashit(get_template_directory_uri()) . $path,
                    [],
